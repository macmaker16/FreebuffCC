    ...
    <div className="h-full flex flex-col bg-dark-950">
      ...
      <div className="px-4 py-3 border-b border-dark-700">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[12px] text-dark-500 ml-auto mr-2">
            {stats.installed} installed · {stats.totalTools} tools active
          </span>
          <button onClick={() => setShowAddModal(true)}
            className="flex items-center gap-1 px-2 py-1 rounded text-[12px] bg-brand-600/20 text-brand-400 hover:bg-brand-600/30 transition-colors">
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <button key={cat.id} onClick={() => setCategory(cat.id)}
              className={`flex items-center gap-1 px-2 py-1 rounded text-[12px] whitespace-nowrap transition-colors ${
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10.5px] px-1.5 py-0.5 bg-dark-800 rounded text-dark-400">v{plugin.version}</span>
                    {plugin.installed && (
                      <span className="text-[10.5px] px-1.5 py-0.5 bg-green-900/30 text-green-400 rounded">Installed</span>
                    )}
                  </div>
                  <p className="text-[12px] text-dark-400 mt-0.5">{plugin.description}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[10.5px] text-dark-500">{plugin.author}</span>
                    <span className="text-[10.5px] text-dark-500">{plugin.downloads.toLocaleString()} downloads</span>
                    <span className="text-[10.5px] text-dark-500">{plugin.tools.length} tools</span>
                  </div>
                  {/* Tool list */}
                  {plugin.installed && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {plugin.tools.map(tool => (
                        <span key={tool.name} className="text-[10.5px] px-1.5 py-0.5 bg-dark-800 rounded text-dark-400 font-mono">
                          {tool.name}
                        </span>
                      ))}
                    </div>
                  ...
                  {plugin.installed ? (
                    <>
                      ...
                      <button onClick={() => handleUninstall(plugin.id)}
                        className="px-2 py-1 rounded text-[12px] bg-red-900/20 text-red-400 hover:bg-red-900/30 transition-colors">
                    ...
                    <div className="flex flex-col gap-1">
                      <button onClick={() => handleInstall(plugin.id)}
                        className="px-3 py-1.5 rounded text-[12px] font-medium bg-brand-600 text-white hover:bg-brand-500 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowAddModal(false)}>
          <div className="bg-dark-900 border border-dark-700 rounded-xl p-4 w-[420px] max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[12px] text-dark-400 block mb-1">Plugin Name *</label>
              ...
              <div>
                <label className="text-[12px] text-dark-400 block mb-1">Description</label>
                <input value={newPlugin.description} onChange={e => setNewPlugin(p => ({ ...p, description: e.target.value }))}
                  className="w-full px-3 py-1.5 bg-dark-800 border border-dark-600 rounded text-xs text-white placeholder-dark-500 focus:outline-none focus:border-brand-500"
                  placeholder="What does this plugin do?" />
              </div>
              <div>
                <label className="text-[12px] text-dark-400 block mb-1">Author</label>
                <input value={newPlugin.author} onChange={e => setNewPlugin(p => ({ ...p, author: e.target.value }))}
                  className="w-full px-3 py-1.5 bg-dark-800 border border-dark-600 rounded text-xs text-white placeholder-dark-500 focus:outline-none focus:border-brand-500"
                  placeholder="your-name" />
              </div>
              <div>
                <label className="text-[12px] text-dark-400 block mb-1">Tools (one per line: name | description)</label>
                {newPlugin.tools.map((tool, i) => (
                  <div key={i} className="flex gap-1 mb-1">
                    ...
                    }} className="flex-1 px-2 py-1 bg-dark-800 border border-dark-600 rounded text-[12px] text-white focus:outline-none focus:border-brand-500" placeholder="tool_name" />
                    <input value={tool.description} onChange={e => {
                      const tools = [...newPlugin.tools]; tools[i] = { ...tools[i], description: e.target.value };
                      setNewPlugin(p => ({ ...p, tools }));
                    }} className="flex-1 px-2 py-1 bg-dark-800 border border-dark-600 rounded text-[12px] text-white focus:outline-none focus:border-brand-500" placeholder="description" />
                  </div>
                ))}
                <button onClick={() => setNewPlugin(p => ({ ...p, tools: [...p.tools, { name: '', description: '' }] }))}
                  className="text-[12px] text-brand-400 hover:text-brand-300">+ Add tool</button>
              </div>
